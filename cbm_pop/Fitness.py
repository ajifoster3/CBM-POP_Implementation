...
class Fitness:
    ...
    @staticmethod
    def fitness_function_locked_tasks(solution, cost_matrix, robot_tasks, initial_robot_cost_matrix, alpha=0.5,
                                    islog=False):
        """
        ...
        try:
            ...
            for agent_idx, task_count in enumerate(agent_task_counts):
                if task_count > 0:
                    ...
                    agent_cost = cost_matrix[robot_tasks[agent_idx]][task_order[counter]]  # Cost from robot to first task
                    if islog:
                        log += f"Initial cost to task {task_order[counter]}: {agent_cost}\n"
                    for i in range(counter, counter + task_count - 1):
                        task_i = task_order[i]
