...
class CBMPopulationAgentOnlineSimpleSimulationLock(Node):
    ...
    def set_coalition_best_solution(self, solution):
        ...
        #print(f"Better solution, im robot {self.agent_ID} and my current task is {self.current_task}")
        if self.current_task is None:
            #print(f"Agent {self.agent_ID} Assigning new task")
    ...
    def assign_next_task(self, solution):
        ...
        try:
            ...
            if solution[1][self.agent_ID] != 0:
                ...
                #print("No Uncovered tasks found!!! ***************************")
                self.current_task = None
            else:
                #print("Agent has No Tasks!!! ***************************")
                self.current_task = None
        except Exception:
    ...
    def solution_update_callback(self, msg):
        ...
        if not self.am_i_failed:
            ...
            if msg is not None and self.coalition_best_solution is not None:
                ...
                their_solution_fitness = Fitness.fitness_function_locked_tasks(received_solution, self.cost_matrix,
                                                                               self.current_tasks,
                                                                               self.robot_initial_pose_cost_matrix)

                our_solution_fitness = Fitness.fitness_function_locked_tasks(self.coalition_best_solution,
                                                                             self.cost_matrix,
                                                                             self.current_tasks,
                                                                             self.robot_initial_pose_cost_matrix)
                ...
                if their_solution_fitness < our_solution_fitness:
                    #print(f"recieved a better solution: their fitness: {their_solution_fitness}, our fitness: {our_solution_fitness}")
    ...
    def remove_extra_tasks(self, solution, target_task_count):
        ...
        # Filter out both already covered and currently allocated tasks
        order = [task for task in order if not self.is_covered[task] and task not in self.current_tasks]
    ...
    def handle_covered_task(self, current_task, is_this_agent=False):
        """
        Handles a task covered by an agent.
        Updating all it's solutions to reflect the new state, and publishing it's new environmental representation.
        *** TODO: This shouldn't always publish! ***
        """
        if self.is_loop_started:
    ...
    def run_step(self):
        ...
        """
        if self.current_task:
            if self.is_covered[self.current_task]:
                self.assign_next_task(self.coalition_best_solution)
        ...
        if self.is_new_task_covered:
            self.handle_covered_task(self.task_covered, True)
            self.task_covered = -1
            self.is_new_task_covered = False
        ...
        if self.current_solution is not None:
            ...
            new_solution_fitness = Fitness.fitness_function_locked_tasks(c_new, self.cost_matrix,
                                                                         self.current_tasks,
                                                                         self.robot_initial_pose_cost_matrix)

            current_solution_fitness = Fitness.fitness_function_locked_tasks(self.current_solution, self.cost_matrix,
                                                                             self.current_tasks,
                                                                             self.robot_initial_pose_cost_matrix)

            if self.local_best_solution:
                local_best_solution_fitness = Fitness.fitness_function_locked_tasks(self.local_best_solution,
                                                                                    self.cost_matrix,
                                                                                    self.current_tasks,
                                                                                    self.robot_initial_pose_cost_matrix)
...
def main(args=None):
    ...
    agent = CBMPopulationAgentOnlineSimpleSimulationLock(
