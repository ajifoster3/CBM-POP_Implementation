...
class CBMPopulationAgentOnlineSimpleSimulationOfflineUCB(Node):
    ...
    def __init__(self, pop_size, eta, rho, di_cycle_length, epsilon, num_iterations,
        ...
        self.T0 = 2.0
        self.T1 = 0.4
        # --- in __init__ of CBMPopulationAgentOnlineSimpleSimulationOffline ---
        # Example for 7 operators; adjust columns to match Operator order
        self.classic_masks = np.array([
            [0, 0, 1, 1, 1, 1],  # c0: initial diversification operator(s)
            [1, 1, 0, 0, 0, 0],  # c1: subsequent intensification operator(s)
            [0, 1, 0, 0, 0, 0],  # c2: which op after intensifier 1
            [1, 0, 0, 0, 0, 0],  # c3: which op after intensifier 2
            [0, 0, 1, 1, 1, 1],  # c4: fallback diversificator set
        ], dtype=float)

        # Gating hyperparams (tweak as you like)
        self.gate_start = 0.80  # start blending masks after 80% of anneal
        self.gate_floor = 0.05  # zeros become a small floor instead of hard zero
        self.use_smooth_blend = True
        '''
    ...
    def run_step(self):
        ...
        if self.current_solution is not None:
            ...
            # Progress along the anneal
            """
            progress = min(1.0, float(self.step_count) / float(max(1, self.anneal_horizon)))
            
            # Base classical mask for the current condition row
            base = self.classic_masks[condition.value]

            # Option A: hard switch near the end
            if not self.use_smooth_blend:
                apply_gate = (progress >= self.gate_start)
                if apply_gate:
                    # soften zeros to a small floor to avoid total lockout
                    mask = self.gate_floor + (1.0 - self.gate_floor) * base
                else:
                    mask = None

            # Option B: smooth blend from identity -> classical mask
            else:
                # alpha goes 0 -> 1 over the last (1 - gate_start) fraction
                if progress <= self.gate_start:
                    alpha = 0.0
                else:
                    alpha = (progress - self.gate_start) / (1.0 - self.gate_start)
                alpha = max(0.0, min(1.0, alpha))

                # identity mask (all ones) blended toward classical mask
                soft_base = self.gate_floor + (1.0 - self.gate_floor) * base  # avoid hard zeros
                mask = (1.0 - alpha) * np.ones_like(soft_base) + alpha * soft_base
