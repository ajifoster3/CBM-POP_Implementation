...
KILL_THRESHOLDS_STR=${KILL_THRESHOLDS_STR:-"0.2"}
ENABLE_KILL=${ENABLE_KILL:-"true"}
NUM_TO_KILL=${NUM_TO_KILL:-"1"}

# turn thresholds string into array — but only matters if kill is enabled
...
# ===== Main: iterate kill thresholds, then runs =====
for KILL_TH in "${KILL_THRESHOLDS[@]}"; do
  if [[ "$ENABLE_KILL" == "true" ]]; then
    COMBO_ROOT="$RESULTS_ROOT/size_${PROBLEM_SIZE}_agents_${NUM_AGENTS}_${PROBLEM_CLASS}_kill_${KILL_TH}_killnum_${NUM_TO_KILL}"
  else
    COMBO_ROOT="$RESULTS_ROOT/size_${PROBLEM_SIZE}_agents_${NUM_AGENTS}_${PROBLEM_CLASS}_kill_disabled"
  fi
  mkdir -p "$COMBO_ROOT"
  ...
  while [ "$run" -le "$NUM_RUNS" ]; do
    ...
    if ! check_coverage_complete "$CONFIG_DIR"; then
      ...
      rm -rf "$CONFIG_DIR"
      echo "[RETRY] Repeating run $run (kill_th=$KILL_TH)"
