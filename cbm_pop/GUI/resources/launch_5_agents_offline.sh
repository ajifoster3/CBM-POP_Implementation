...
NODE_NAME="cbm_population_agent_offline"
...
cleanup() {
    ...
    pkill -f "cbm_population_agent_offline" 2>/dev/null
    pkill -f "cbm_fitness_logger_offline" 2>/dev/null
    sleep 1
...
for RUN in {1..5}; do
    ...
    ros2 run "$PACKAGE_NAME" cbm_fitness_logger_offline --ros-args -p timeout:=$TRACKING_TIMEOUT &
    PIDS+=($!)
    ...
    while true; do
        ACTIVE_PROCESSES=$(pgrep -f "cbm_population_agent_offline|cbm_fitness_logger_offline|llm_interface_agent")
    ...
    if pgrep -f "cbm_population_agent_offline" || pgrep -f "cbm_fitness_logger_offline"; then
        echo "WARNING: Some processes are still running! Manually killing them..."
        pkill -9 -f "cbm_population_agent_offline"
        pkill -9 -f "cbm_fitness_logger_offline"
    fi
